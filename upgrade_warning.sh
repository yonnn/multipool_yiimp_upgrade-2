...
\n\nDo to so many custom YiiMP installations this installer will ONLY upgrade the required stratum files.
\n\nIf your pool has coins that require modifiaction to coinbase.cpp, please submit a request on our github at
\n\nhttps://github.com/cryptopool-builders/yiimp with the required code and we will add it to the stratum build.
\n\n
\n\nThis upgrade also will NOT copy any example.stratum.conf files to /home/crypto-data/yiimp/site/stratum/config.
\n\nThis is done to prvent any overwrites of any dedicated port configs you might have. If you need a config file for a new
\n\nAlgo, please visit our github for the example.
